 ...
 public static boolean findPattern( int patternIndex, int stringIndex , int patternSize, int stringSize ){
	...
	if(patternIndex == patternSize){
	 ...
	 if(pattern.charAt(patternIndex)=='*'){
		 // we have to case either 1. consider current character of string 2. leave current char in string.  
    return (findPattern(patternIndex+1, stringIndex, patternSize, stringSize) || findPattern(patternIndex, stringIndex+1, patternSize, stringSize));
