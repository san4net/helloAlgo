...
public class BSTTree<T>
    ...
    public static void main(String[] args)
        ...
        node.add(1);
        node.add(5);
        BSTTree<Integer> tree = (BSTTree<Integer>) buildTree(node);
    ...
    public void printLevelTraversal()
        ...
        while (!queue.isEmpty()) {
            ...
            // now traverse print and add them back to queue
            for (TreeNode<T> node : nodes) {
                System.out.println(node.getData());
                if (node.getLeft() != null)
                    queue.enqueue((TreeNode<T>) node.getLeft());
                if (node.getRight() != null) {
                    queue.enqueue((TreeNode<T>) node.getRight());
                }
            }
        }
    }
